...
const bottomwear = ({ products, subCategory, mainCategory }) => {
  ...
  const filteredProduct = products.filter((item) => {
    return (
      ...
      item.subCategory._ref ===
        subCategory.find((cat) => cat.title === "Womens bottom wear")._id
    );
  });
  return (
    <div>
      ...
      <ProductList title="Bottom Wear Products" products={filteredProduct} />
      <Footer />
    </div>
  );
};
